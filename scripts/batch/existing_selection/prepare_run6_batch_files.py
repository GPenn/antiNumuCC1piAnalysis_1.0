...
launchscriptfile = open("/user/gpenn/nd280software/nd280release_12.31/antiNumuCC1piAnalysis_1.0/scripts/batch/existing_selection/run6/submit_batch_jobs_run6.sh", 'w')
...
for list in lists:
    count += 1
    shname = "/user/gpenn/nd280software/nd280release_12.31/antiNumuCC1piAnalysis_1.0/scripts/batch/existing_selection/run6/submit_run6_" + str(count) + ".sh"
    ...
    shfile.write("RunAntiNumuCCMultiPiAnalysis.exe " + list.strip() + " -o output/existing_selection/run6/oaAnalysis_prod6Trun6_" + str(count) + ".root\n")
    shfile.close()
